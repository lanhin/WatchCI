...
_apply_global_defaults() {
  ...
  # strip trailing slash so comment links are predictable
  SITE_PUBLIC_URL="${SITE_PUBLIC_URL%/}"
  ...
  export SITE_DIR PUBLISH_CMD AUTO_PUBLISH SITE_PUBLIC_URL
...
_ENV_OVERRIDE_KEYS=(
  ...
  POLL_INTERVAL_SEC DATA_DIR SITE_DIR PUBLISH_CMD AUTO_PUBLISH SITE_PUBLIC_URL
...
load_project_file() {
  ...
  POST_PR_COMMENT=false SKIP_IF_PR_SUCCESS_COMMENT=false
  ...
  SKIP_IF_PR_SUCCESS_COMMENT="${SKIP_IF_PR_SUCCESS_COMMENT:-false}"
  ...
  export ALLOW_MANUAL_RERUN POST_PR_COMMENT SKIP_IF_PR_SUCCESS_COMMENT POLL_INTERVAL_SEC
